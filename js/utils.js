...
// v49-3(docs/指示書/v49-3_時計と処理能力の物差し_指示書.md §2-3): **1700 → 37500 に変えた。**
// v49 でプレイヤーが自分の手で1杯を作るようになり、時計を「手の速さ」に合わせる必要が出た。
// 物差しは「一番弱いスタッフの1杯 = 8ゲーム分」(js/scoring.js の minutesPerBowl)で、
// **8ゲーム分 = 5.0秒**(= v49-2 で実測したプレイヤーの1杯の実時間)になる値がこれ:
//   37500 ÷ 60 × 8 = 5000ms。1ゲーム時間 = 37.5秒。
// v49-3 §1 の調査どおり、歩きはゲーム分で値段が付いているので、この値を変えても
// 「1杯が何ゲーム分か」は動かない(動くのは実秒の体感だけ)。1杯を8分に収める側は
// shop-view.js の WALK_MIN_PER_CELL / 調理の尺が担当する。
window.BASE_HOUR_MS = 37500; // ×1 = 1時間37.5秒(1ゲーム分 = 625ms)
// v49-3(§2-3): 閑散時の自動早送りの倍率。**店内に客0・行列0・プレイヤーが手あき**のときだけ
// 自動でこの倍率になる(js/screens/loop.js の quiet watcher)。20 = 1ゲーム時間 1.875秒で、
// v49-2 までの速さ(1.7秒)とほぼ同じ——「誰も居ない時間は今までどおりの速さで飛ばす」。
// 手動の ×1/×2/×4 は閑散中は無視する(この倍率が勝つ)。停止(×0)のときは早送りしない。
window.IDLE_FAST_MULT = 20;
