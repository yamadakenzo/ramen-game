...
// v49-4(docs/指示書/v49-4_シフト時計_指示書.md §1): **1日 = 1シフト。**
// v49-3 の「時計を8ゲーム分の物差しに合わせる(37500)」は撤回した——1週が最大52分になり、
// 遊びのテンポとして持たなかった(v49-3 実装報告 §6-1)。
// 代わりに「1日をひとまとまりの短いシフトとして遊ぶ」に置き直した:
//   営業は 11時→23時 の12時間。**この1つの値だけが時計の出典**(2段の速さも自動早送りも無い)。
// v49-6(docs/指示書/v49-6_テンポ調整と食事ゲージ_指示書.md §2): **1日 90秒 → 60秒。**
// 実機の所見は「歩きが速すぎて何が起きているか読めないのに、全体のテンポは遅い」だった。
// そこで役割を分けた——**動きの速さは遅く(shop-view.js の実ms定数)、1日の長さはここで短く**。
// 12時間 = 60秒なので 1ゲーム時間 = 5秒、×4 なら1日15秒。客の湧きの配分(openBand)と
// 間合いの尺(gm())はこの1つの値に自動で追随する(配分のロジック自体は変えていない)。
// v49-4(§0): **minutesPerBowl(js/scoring.js)は週の計算にだけ使う。** 絵と操作の速さは
// この時計とは切り離し、js/screens/shop-view.js の実ms定数で持つ。
window.BASE_HOUR_MS = 5000; // ×1 = 1時間5秒 → 1日(12時間) = 60秒
